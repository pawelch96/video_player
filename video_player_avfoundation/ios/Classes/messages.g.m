...
- (NSDictionary *)toMap;
@end
@interface PictureInPictureMessage ()
+ (PictureInPictureMessage *)fromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end
...
@implementation PictureInPictureMessage
+ (instancetype)makeWithTextureId:(nullable NSNumber *)textureId
  ...
  PictureInPictureMessage* pigeonResult = [[PictureInPictureMessage alloc] init];
  pigeonResult.textureId = textureId;
...
- (nullable id)readValueOfType:(UInt8)type 
{
  switch (type) {
    ...
    case 131:     
      return [PictureInPictureMessage fromMap:[self readValue]];
...
- (void)writeValue:(id)value 
{
  ...
  if ([value isKindOfClass:[PictureInPictureMessage class]]) {
    [self writeByte:131];
    [self writeValue:[value toMap]];
  } else 
...
void VideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<VideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.VideoPlayerApi.setPictureInPicture"
        binaryMessenger:binaryMessenger
        codec:VideoPlayerApiGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setPictureInPictureMsg:error:)], @"VideoPlayerApi api (%@) doesn't respond to @selector(setPictureInPictureMsg:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        PictureInPictureMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api setPictureInPictureMsg:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    }
    else {
